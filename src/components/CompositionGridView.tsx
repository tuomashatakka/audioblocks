...
const CompositionGridView: React.FC<CompositionGridViewProps> = ({
  ...
  const getTrackEditingUserId = useCallback((trackIndex: number) => blocks.find(
    block => block.track === trackIndex &&
               block.editingUserId &&
               block.editingUserId !== localUserId
  )?.editingUserId || null, [ blocks, localUserId ])
  ...
  const isTrackLocked = useCallback((trackIndex: number): boolean => blocks.some(block =>
  ...
  const handleFileUpload = useCallback((files: FileList | null) => {
    if (!files || !contextMenuPosition || !onUploadAudio)
      return
          ...
          <div
            ...
            <div
              ...
              {tracks.map((_, index) => {
                ...
                return <div key={ index } className='track-edited-by-user absolute left-0 right-0'>
